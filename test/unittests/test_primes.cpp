...
{
    /// Test pairs for find_largest_prime() are taken from generated frkhash
    /// light cache and full dataset sizes, picked 50 at random from each collection.
    /// See https://github.com/ethereum/wiki/wiki/Frkhash#data-sizes.
    ...
    {
        int prime = frkhash_find_largest_prime(t.first);
...
{
    EXPECT_EQ(frkhash_find_largest_prime(-2147483647), 0);
    EXPECT_EQ(frkhash_find_largest_prime(-100), 0);
    EXPECT_EQ(frkhash_find_largest_prime(-1), 0);
    EXPECT_EQ(frkhash_find_largest_prime(0), 0);
    EXPECT_EQ(frkhash_find_largest_prime(1), 0);
    EXPECT_EQ(frkhash_find_largest_prime(2), 2);
    EXPECT_EQ(frkhash_find_largest_prime(3), 3);
    EXPECT_EQ(frkhash_find_largest_prime(4), 3);
    EXPECT_EQ(frkhash_find_largest_prime(5), 5);
    EXPECT_EQ(frkhash_find_largest_prime(6), 5);
    EXPECT_EQ(frkhash_find_largest_prime(7), 7);
}
